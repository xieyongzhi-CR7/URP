...
#elif defined(_OTHER_PCF5)
// 需要16个滤波样本
#define _OTHER_FILTER_SAMPLES 16
...
// 混合光照  第一个参数：是positionWS和maxShadowDistance的比例算出的宏观的阴影强度 ，     第二个参数  shadow : 是传进来的实时阴影（如果是shadowMask模式， 则使用烘焙阴影替换实时阴影）
...
float GetOtherShadowAttenuation(OtherShadowData other,ShadowData global,Surface surfaceWS)
{
    ...
    else
    {
        ...
        // global 全局强度：用于确定是否可以跳过采样实时阴影 ， 比如超出阴影距离, 或者是在最大的级联范围之外，
        //  但是级联只用于定向阴影
        
        //  其它阴影有固定的位置  所以级联没有意义，阴影不会随view而改变
        
        // 但是  还是对所有对象都使用 相同的全局强度
